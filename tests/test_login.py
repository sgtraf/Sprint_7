...
class TestLogin:
    ...
    def test_courier_authorization(self,generate_login_data):
        with allure.step("Создаем курьера"):
            LoginMethods.register_new_courier_and_return_login_password(generate_login_data)
        with allure.step("Входим под логином курьера"):
            body = LoginMethods.login_in_system(generate_login_data['login'], generate_login_data['password'])
    ...
    def test_login_courier_without_password(self, generate_login_data):
        with allure.step("Создаем курьера"):
            LoginMethods.register_new_courier_and_return_login_password(generate_login_data)
        with allure.step("Пытаемся войти без указания пароля"):
            body_answer_without_pass = LoginMethods.login_in_system(generate_login_data['login'], '')
        with allure.step("Проверяем, что код ответа и тело соответствует документации"):
    ...
    def test_login_with_incorrect_password(self,generate_login_data):
        with allure.step("Создаем курьера"):
            LoginMethods.register_new_courier_and_return_login_password(generate_login_data)
        with allure.step("Пытаемся войти с неправильным паролем"):
            body_answer_withot_pass = LoginMethods.login_in_system(generate_login_data['login'], '4545')
        with allure.step("Проверяем, что код ответа и тело соответствует документации"):
    ...
    def test_login_courier_without_password_error(self, generate_login_data):
        with allure.step("Создаем курьера"):
            LoginMethods.register_new_courier_and_return_login_password(generate_login_data)
        with allure.step("Пытаемся войти без указания пароля"):
            body_answer_without_pass = LoginMethods.login_in_system(generate_login_data['login'], '')
    ...
    def test_login_with_real_data(self, generate_login_data):
        with allure.step("Создаем курьера"):
            LoginMethods.register_new_courier_and_return_login_password(generate_login_data)
        with allure.step("Входим под логином курьера"):
            body = LoginMethods.login_in_system(generate_login_data['login'], generate_login_data['password'])
